...
/**
 * Determines the post status based on the STATE_ID from the API
 * Draft: 1, 7, 8, 9, 10
 * Publish: 2, 4, 5, 6
 * Default: draft (for safety)
 */
function determine_post_status($api_publication) {
    $state_id = isset($api_publication['STATE_ID']) ? (int) $api_publication['STATE_ID'] : null;
    
    if ($state_id === null) {
        return 'draft'; // Default to draft if STATE_ID is missing
    }
    
    $publish_states = [2, 4, 5, 6];
    $draft_states = [1, 7, 8, 9, 10];
    
    if (in_array($state_id, $publish_states)) {
        return 'publish';
    } elseif (in_array($state_id, $draft_states)) {
        return 'draft';
    }
    
    // For any other STATE_ID values, default to draft for safety
    return 'draft';
}

/**
 * Formats a date for ACF Date Picker field (expects Y-m-d format)
 * API format: "June, 02 2006 14:26:07" or similar
 * ACF Date Picker format: "2006-06-02"
 */
function format_date_for_acf_date_picker($api_date) {
    if (empty($api_date)) {
        return null;
    }
    ...
    try {
        ...
        // Return in ACF Date Picker format (Y-m-d)
        return $date->format('Y-m-d');
        
    } catch (Exception $e) {
        error_log('ACF Date Picker format error: ' . $e->getMessage());
        return null;
    }
}