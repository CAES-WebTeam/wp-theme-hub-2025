...
CSS;
        ...
        switch ($action) {
            ...
            case 'run_all_syncs':
                ...
                output_sync_message('✨ Beginning all scheduled user data synchronization operations...', 'info');

                // 1. Sync Active Personnel
                output_sync_message('➡️ Step 1 of 2: Syncing Active Personnel from primary external API...', 'info');
                $result = sync_personnel_users();
                if (is_wp_error($result)) {
                    output_sync_message('❌ Step 1 Error (Active Personnel): ' . $result->get_error_message(), 'error');
                } else {
                    output_sync_message('✅ Step 1 Complete (Active Personnel): ' . $result['message'], 'success');
                }
                display_import_error_summary('Step 1 - Active Personnel Sync');

                // 2. Sync Inactive Personnel
                output_sync_message('➡️ Step 2 of 2: Syncing Inactive/Archived Personnel from secondary external API...', 'info');
                $result = sync_personnel_users2();
                if (is_wp_error($result)) {
                    output_sync_message('❌ Step 2 Error (Inactive Personnel): ' . $result->get_error_message(), 'error');
                } else {
                    output_sync_message('✅ Step 2 Complete (Inactive Personnel): ' . $result['message'], 'success');
                }
                display_import_error_summary('Step 2 - Inactive Personnel Sync');

                output_sync_message('🎉 All user data sync operations have finished!', 'success');
                break;
        }
