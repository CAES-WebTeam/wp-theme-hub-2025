...
/**
 * TEMPORARY DEBUG FUNCTION - DELETE THIS ENTIRE FUNCTION AFTER FIXING
 */
function debug_authors_data() {
    // Only run on singular publication pages when logged in
    if (!is_singular('publications') || !is_user_logged_in()) {
        return;
    }
    ...
    $authors_data = get_field('authors', $post->ID);
    
    echo '<div style="background: #f0f0f0; padding: 20px; margin: 20px; border: 2px solid #000; font-family: monospace; position: relative; z-index: 9999;">';
    echo '<h3>DEBUG: Authors Raw Data</h3>';
    echo '<pre>' . print_r($authors_data, true) . '</pre>';
    echo '<h3>DEBUG: Processing Each Author</h3>';
    ...
    if ($authors_data && is_array($authors_data)) {
        foreach ($authors_data as $index => $item) {
            ...
            echo '<strong>Author #' . ($index + 1) . ':</strong><br>';
            echo 'Type: ' . ($item['type'] ?? 'NOT SET') . '<br>';
            
            $entry_type = $item['type'] ?? '';
            
            if ($entry_type === 'Custom') {
                $custom_user = $item['custom_user'] ?? $item['custom'] ?? array();
                echo 'First Name: ' . ($custom_user['first_name'] ?? 'EMPTY') . '<br>';
                echo 'Last Name: ' . ($custom_user['last_name'] ?? 'EMPTY') . '<br>';
            } else {
                // Try to find user ID
                $user_id = null;
                ...
                if (isset($item['user']) && !empty($item['user'])) {
                    ...
                    echo 'User ID (from user key): ' . ($user_id ?? 'NULL') . '<br>';
                }
                
                if (empty($user_id) && is_array($item)) {
                    echo 'Searching for numeric value in item...<br>';
                    foreach ($item as $key => $value) {
                        echo '  - Key "' . $key . '" = ' . print_r($value, true) . '<br>';
                ...
                if ($user_id && is_numeric($user_id) && $user_id > 0) {
                    $email_acf = get_field('field_uga_email_custom', 'user_' . $user_id);
                    $email_wp = get_the_author_meta('user_email', $user_id);
                    $display_name = get_the_author_meta('display_name', $user_id);
                    
                    echo 'Display Name: ' . ($display_name ?? 'NONE') . '<br>';
                    echo 'ACF Email: ' . ($email_acf ?? 'EMPTY') . '<br>';
                    echo 'WP Email: ' . ($email_wp ?? 'EMPTY') . '<br>';
                    
                    $email = $email_acf ?: $email_wp;
                    ...
                    if ($email && !strpos($email, 'placeholder')) {
                        ...
                        if (count($email_parts) === 2) {
                            ...
                            echo 'Contains "spoofed"? ' . (strpos($domain, 'spoofed') !== false ? 'YES' : 'NO') . '<br>';
                            if (strpos($domain, 'spoofed') === false) {
                                $username = $email_parts[0];
                                echo '✓ WOULD ADD USERNAME: ' . $username . '<br>';
                            }
                        }
                    } else {
...
add_action('wp_body_open', 'debug_authors_data');
