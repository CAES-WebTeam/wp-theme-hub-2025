...
window.addEventListener('load', function () {
  ...
  function scrollToTarget(targetY) {
    ...
    if (distance > SMOOTH_SCROLL_THRESHOLD) {
      ...
      window.scrollTo({
        top: Math.max(0, targetY - LEAD_IN_DISTANCE),
        behavior: 'instant'
      });
  ...
  function assignHeadingIDs() {
    ...
    const headingsInDom = Array.from(postContent.querySelectorAll(showSubheadings ? 'h2, h3, h4, h5, h6' : 'h2')).filter(heading => {
    ...
    headingsInDom.forEach(domHeading => {
      ...
      const matchingData = currentPageHeadings.find(data => normalizeText(data.text) === headingText);
      if (matchingData) {
